...
-- 3.15 ĐỀ TÀI
-- Tìm đợt đồ án đang hoạt động (trang_thai = 1)
DECLARE @id_dot_active INT = (SELECT TOP 1 id FROM DotDoAn WHERE trang_thai = 1);

-- Nếu không tìm thấy đợt active, lấy đợt mới nhất
IF @id_dot_active IS NULL
    SET @id_dot_active = (SELECT TOP 1 id FROM DotDoAn ORDER BY id DESC);
...
-- 3.21 CẤU HÌNH PHIẾU CHẤM CHO ĐỢT ĐỒ ÁN ĐANG HOẠT ĐỘNG
-- Tìm đợt đồ án đang hoạt động (trang_thai = 1), nếu không có thì lấy đợt mới nhất
DECLARE @id_dot_active INT = (SELECT TOP 1 id FROM DotDoAn WHERE trang_thai = 1);
IF @id_dot_active IS NULL
    SET @id_dot_active = (SELECT TOP 1 id FROM DotDoAn ORDER BY id DESC);
...
PRINT N'=== CẤU HÌNH PHIẾU CHẤM ===';
PRINT N'ID Đợt đồ án: ' + ISNULL(CAST(@id_dot_active AS NVARCHAR(10)), 'NULL');
PRINT N'ID Phiếu giữa kì: ' + ISNULL(CAST(@id_phieu_giuaki AS NVARCHAR(10)), 'NULL');
PRINT N'ID Phiếu GVHD: ' + ISNULL(CAST(@id_phieu_gvhd AS NVARCHAR(10)), 'NULL');
PRINT N'ID Phiếu Phản biện: ' + ISNULL(CAST(@id_phieu_phanbien AS NVARCHAR(10)), 'NULL');
PRINT N'ID Phiếu Hội đồng: ' + ISNULL(CAST(@id_phieu_hd AS NVARCHAR(10)), 'NULL');
...
-- Lấy ID tiêu chí phiếu giữa kì (chỉ nhận xét)
DECLARE @tc_gk_1 INT = (SELECT TOP 1 id FROM TieuChiChamDiem WHERE ten_tieu_chi LIKE N'%Tiến độ thực hiện%');
DECLARE @tc_gk_2 INT = (SELECT TOP 1 id FROM TieuChiChamDiem WHERE ten_tieu_chi LIKE N'%Phương pháp và cách tiếp cận%');
DECLARE @tc_gk_3 INT = (SELECT TOP 1 id FROM TieuChiChamDiem WHERE ten_tieu_chi LIKE N'%Kết quả sơ bộ%');
DECLARE @tc_gk_4 INT = (SELECT TOP 1 id FROM TieuChiChamDiem WHERE ten_tieu_chi LIKE N'%Thái độ và tinh thần%');
DECLARE @tc_gk_5 INT = (SELECT TOP 1 id FROM TieuChiChamDiem WHERE ten_tieu_chi LIKE N'%Đề xuất và góp ý%');

-- Điểm chi tiết — SV 13 (Ngô Thị Tuyết Như), giữa kỳ (GV 7 - Trần Văn Hùng chấm) — chỉ nhận xét, diem_so = 0
INSERT INTO DiemChiTiet (id_phien_bao_ve, id_nguoi_cham, id_sinh_vien, id_tieu_chi, diem_so, nhan_xet) VALUES
(@id_pbv_gk, 7, 13, @tc_gk_1, 0, N'Tiến độ đạt khoảng 60% so với kế hoạch, cần đẩy nhanh phần tích hợp phần cứng.'),
(@id_pbv_gk, 7, 13, @tc_gk_2, 0, N'Phương pháp nghiên cứu hợp lý, đã áp dụng đúng quy trình phát triển phần mềm.'),
(@id_pbv_gk, 7, 13, @tc_gk_3, 0, N'Đã hoàn thành module đọc RFID và giao diện cơ bản. Cần bổ sung thêm tính năng quản lý.'),
(@id_pbv_gk, 7, 13, @tc_gk_4, 0, N'Sinh viên chủ động liên hệ GVHD, chuyên cần và tuân thủ lịch hẹn.'),
(@id_pbv_gk, 7, 13, @tc_gk_5, 0, N'Nên bổ sung phần kiểm thử tự động và viết tài liệu kỹ thuật song song với lập trình.');
...
-- Kết quả bảo vệ — chỉ áp dụng cho hội đồng cuối kỳ (giữa kỳ chỉ nhận xét)
...
-- 3.33 XÁC NHẬN ĐIỂM CHỦ TỊCH (chỉ áp dụng cho hội đồng cuối kỳ)
...
-- =====================================================================
-- PHẦN 4: STORED PROCEDURES HỖ TRỢ
-- =====================================================================

-- Stored Procedure: Tự động tạo cấu hình phiếu chấm cho đợt đồ án mới
-- Sử dụng khi tạo đợt đồ án mới hoặc khi cần sao chép cấu hình từ đợt cũ
CREATE OR ALTER PROCEDURE sp_TaoCauHinhPhieuCham_ChoDot
    @id_dot INT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    IF NOT EXISTS (SELECT 1 FROM DotDoAn WHERE id = @id_dot)
    BEGIN
        RAISERROR(N'Đợt đồ án không tồn tại!', 16, 1);
        RETURN;
    END

    -- Xóa cấu hình cũ của đợt (nếu có)
    DELETE FROM CauHinhPhieuCham_Dot WHERE id_dot = @id_dot;

    -- Lấy ID các loại phiếu chấm
    DECLARE @id_phieu_giuaki INT = (SELECT TOP 1 id FROM LoaiPhieuCham WHERE ten_loai_phieu LIKE N'%giữa kì%' OR ten_loai_phieu LIKE N'%giữa kỳ%');
    DECLARE @id_phieu_gvhd INT = (SELECT TOP 1 id FROM LoaiPhieuCham WHERE ten_loai_phieu LIKE N'%hướng dẫn%');
    DECLARE @id_phieu_phanbien INT = (SELECT TOP 1 id FROM LoaiPhieuCham WHERE ten_loai_phieu LIKE N'%Phản biện%');
    DECLARE @id_phieu_hd INT = (SELECT TOP 1 id FROM LoaiPhieuCham WHERE ten_loai_phieu LIKE N'%Hội đồng%');

    -- Tạo cấu hình phiếu chấm cho đợt
    IF @id_phieu_giuaki IS NOT NULL
        INSERT INTO CauHinhPhieuCham_Dot (id_dot, vai_tro_cham, id_loai_phieu) VALUES (@id_dot, 'GVHD_GIUA_KY', @id_phieu_giuaki);

    IF @id_phieu_gvhd IS NOT NULL
        INSERT INTO CauHinhPhieuCham_Dot (id_dot, vai_tro_cham, id_loai_phieu) VALUES (@id_dot, 'GVHD', @id_phieu_gvhd);

    IF @id_phieu_phanbien IS NOT NULL
        INSERT INTO CauHinhPhieuCham_Dot (id_dot, vai_tro_cham, id_loai_phieu) VALUES (@id_dot, 'PhanBien', @id_phieu_phanbien);

    IF @id_phieu_hd IS NOT NULL
        INSERT INTO CauHinhPhieuCham_Dot (id_dot, vai_tro_cham, id_loai_phieu) VALUES (@id_dot, 'HoiDong', @id_phieu_hd);

    PRINT N'Đã tạo cấu hình phiếu chấm cho đợt ID: ' + CAST(@id_dot AS NVARCHAR(10));
END;
GO

-- Stored Procedure: Kiểm tra và tự động tạo cấu hình cho tất cả đợt đang hoạt động chưa có cấu hình
CREATE OR ALTER PROCEDURE sp_KiemTraVaTaoCauHinhPhieuCham
AS
BEGIN
    SET NOCOUNT ON;
    ...
    -- Cursor để duyệt qua các đợt đang hoạt động chưa có cấu hình
    DECLARE dot_cursor CURSOR FOR
        SELECT d.id
        FROM DotDoAn d
        WHERE d.trang_thai = 1
          AND NOT EXISTS (SELECT 1 FROM CauHinhPhieuCham_Dot c WHERE c.id_dot = d.id);

    OPEN dot_cursor;
    FETCH NEXT FROM dot_cursor INTO @id_dot;

    WHILE @@FETCH_STATUS = 0
    BEGIN
        EXEC sp_TaoCauHinhPhieuCham_ChoDot @id_dot;
        FETCH NEXT FROM dot_cursor INTO @id_dot;
    END;

    CLOSE dot_cursor;
    DEALLOCATE dot_cursor;

    PRINT N'Hoàn thành kiểm tra và tạo cấu hình phiếu chấm!';
END;
GO

-- Chạy kiểm tra và tự động tạo cấu hình cho các đợt chưa có
EXEC sp_KiemTraVaTaoCauHinhPhieuCham;
GO
...
PRINT N'Stored Procedures: sp_TaoCauHinhPhieuCham_ChoDot, sp_KiemTraVaTaoCauHinhPhieuCham';
PRINT N'=====================================================================';
